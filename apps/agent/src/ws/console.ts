...
export async function consoleAgentWs(app: FastifyInstance): Promise<void> {
  app.get(
    ...
    async (connection, req) => {
      ...
      if (!container) {
        connection.socket.close(4404, "no container");
        return;
      }
      ...
      logs.on("data", (chunk: Buffer) => {
        // Docker multiplexes logs when there is no TTY. Strip the 8-byte header
        // per frame; the remainder is the UTF-8 payload.
        let offset = 0;

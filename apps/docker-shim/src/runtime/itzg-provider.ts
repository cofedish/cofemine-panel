...
import { CA_MOUNT_PATH, CA_PUB_VOLUME_NAME } from "../ca-volumes.js";

const IMAGE = config.MC_IMAGE;
  ...
  createContainerSpec(spec: ServerSpec, dataPath: string): ContainerCreateOptions {
    ...
    const installRetryDefaults: Record<string, string> =
      spec.type === "MODRINTH" || spec.type === "CURSEFORGE"
        ? {
            MAX_RETRIES: config.MC_MAX_RETRIES,
            RETRY_BACKOFF: config.MC_RETRY_BACKOFF,
          }
        : {};
    ...
    const cacheHost = config.MAVEN_CACHE_HOST;
    if (cacheHost) {
    ...
    const labels: Record<string, string> = {
      [`${config.LABEL_PREFIX}.managed`]: "true",
      [`${config.LABEL_PREFIX}.serverId`]: spec.id,
      [`${config.LABEL_PREFIX}.name`]: spec.name,
      [`${config.LABEL_PREFIX}.type`]: spec.type,
      [`${config.LABEL_PREFIX}.version`]: spec.version,
    };
